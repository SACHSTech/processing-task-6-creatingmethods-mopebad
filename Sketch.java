...
public class Sketch extends PApplet {
  ...
  public void setup() {
    background(51, 102, 255); // Set the background color
  }

  public void draw() {
    // Check if a mouse button has been pressed
    if (TurnWhite(mousePressed)) {
      // Draw white houses and stick figures when the mouse is pressed
      for (int intY = 100; intY < 750; intY += 300) {
        for (int intX = 50; intX < 1000; intX += 300) {
          drawHouse(intX, intY, 255, 255, 255); // Use white color for the house
  ...
  public void drawHouse(int intHouseX, int intHouseY, int intRed, int intGreen, int intBlue) {
    ...
    // Roof of the house
    stroke(0);
    fill(255, 204, 0); // Change roof color
